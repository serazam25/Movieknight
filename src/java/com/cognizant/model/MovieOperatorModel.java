...
public class MovieOperatorModel {
	...
	public returnClass insertRegistrationData() {
		...
		this.operatorID = idg.generateId("OperatorId", "Movieoperator");
		r.setS(this.operatorID);
		PreparedStatement pstmt = null;

		String query = "insert into Movieoperator(OperatorId,OperatorName,Address,EMail,City,PhoneNumber,Password) values(?,?,?,?,?,?,?)";
	...
	public boolean selectLoginData() throws SQLException {
		...
		String query = "select * from Movieoperator where EMail='" + this.email+ "' and password='" + this.password + "';";
...
public ResultSet selectLoginData1() {
	...
	public boolean operator_available(String s) {
		DBConnector dbc = new DBConnector();
		String query = "select OperatorId from Movieoperator";
		ResultSet rs = null;
		rs = dbc.fireExecuteQuery(query);
		boolean result = false;
		try {
	...
	public boolean deleteOperatorAccount() {
		...
		String query = "create TRIGGER beforeOperatorDelete"
				+ "BEFORE DELETE ON Movieoperator" + "FOR EACH ROW"
				+ "INSERT INTO operatorhistory" + "select * from user;";
		String query1 = "delete from Movieoperator where email='"
	...
	// Method created by basha. consist of code for changing password for Movie
	// operator
	public boolean updatePasswordByEMail() {
		// TODO Auto-generated method stub
		DBConnector dbc = new DBConnector();
		String query = "update Movieoperator set password = '" + this.password
	...
	public Boolean deleteOperatorAccountFromAdmin() {
		...
		String query = "create TRIGGER beforeOperatorDelete"
				+ "BEFORE DELETE ON Movieoperator" + "FOR EACH ROW"
				+ "INSERT INTO operatorhistory" + "select * from user;";
		String query1 = "delete from Movieoperator where email='" + this.email
	...
	public String getUserIDFromEmail(String email2) throws SQLException {
		String query = "select operatorID from Movieoperator where email='"
				+ email2 + "';";
		System.out.println(query);
		DBConnector dbc = new DBConnector();
	...
	public ResultSet getOperatorData() {
		String query = "select * from Movieoperator where operatorid='"
				+ this.operatorID + "';";
	...
	public boolean updateUserProfileData() {
		String query = "update Movieoperator set OperatorName = '" + this.name
