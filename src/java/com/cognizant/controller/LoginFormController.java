...
public class LoginFormController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (!errorMessage.equalsIgnoreCase("")) {
			...
			else if(type.equalsIgnoreCase("Movieoperator")){
				response.sendRedirect("MovieOperatorLogin.jsp?msg="+errorMessage);
			}
			else{
			...
			if (type.equalsIgnoreCase("Movieoperator")) {
				...
				try {
					if (bom.selectLoginData()) {
						...
						response.sendRedirect("index_committee_member.jsp");
					} else {
						msg = msg + " User password is incorrect";
						response.sendRedirect("MovieOperatorLogin.jsp?msg=" + msg);
					}
